extrapolate_empirical_ecov <- function(coef_odiag,coef_diag,dti,dtf,ntot){

# PURPOSE:
#   This subroutine extrapolates the empirical fit of the error covariance found for
#   the initalization frequency 'dti' to a finer initalization frequency (dtf). 
#
#   The off-diagonal fit is assumed to be an exponentially decreasing
#   function of the form:
#              C(i,j) = a_min[i,j]*exp(-1*(gamma_min[i,j])*|i-j|)+b_min[i,j]
#   Where: 
#   i,j == are index values of the lagged error covariance matrix,
#          contingent on i not being equal to j;
#
...
#
#   The diagonal elements are modeled by a logistic funtion, fit to the residuals between 
#   the diagonal elements of the error covariance matrix and the off diagaonal elements. 
...
# REFERENCES: 
#  Lorenz, E. N. (1982), Atmospheric predictaiblity experiments with a large        
#  numerical model, Tellus, 34, 505â€“513.
#
# CALLING SEQUENCE:
#    Cn <- extrapolate_empirical_ecov(coef_odiag,coef_diag,dti,dtf,ntot);
#
# INPUTS:
#       coef_odiag: The parameters a, gamma, and b from the nonlinear fit of the 
#                   off-diagonal elements of the lagged error covariance matrix. 
#       coef_diag:  The parameters epsilon, alpha, and tau0 recovered when the 
#                   diagonal of the lagged error covariance matrix is fit as a logistics equation. 
#       dti:  The initalization frequency for data used in estimating the empirical 
#             fit of the error covariance matrix. For example if using a daily 
#             initalization frequency, dt =1. If initalized every five days, dt = 5. 
#       dtf:  The new initalization frequency for which the emprical fit
#             will be extrapolated. 
#       ntot: The total number of leads times.

# OUTPUTS:
#       Cn$C: The [ntot  xntot] Empirically derived lagged error covariance matrix       
#         extroplated to have an initalization frequency of dtf. 
#        
#		C$iday and C$jday: The dimensions of the lagged error covariance matrix.

ntt <- ntot/dtf
Cn <- array(,c(ntt,ntt))
t = seq(dtf/dti,ntot,dtf/dti)
...
at = coef_odiag[1,1]*t+coef_odiag[2,1];
bt = coef_odiag[1,3]*t+coef_odiag[2,3];
gt = coef_odiag[1,2]*t;


for (n in 1:(ntt-1)){
	tt =t[1:(ntt-n)]
	#tt =seq(dtf,(ntt-1),dtf)
	ft <-array(,c(ntt,1))
...
d = coef_diag[1,1]/(1+exp(-coef_diag[1,2]*(t-coef_diag[1,3])));
     dt = d+at+bt;
 ...
 iday = seq(dtf,ntot,dtf)
 jday = seq(dtf,ntot,dtf)
 
 result<-list(C= Cn,iday=iday,jday=jday)
 return(result)
